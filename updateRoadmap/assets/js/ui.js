...
boxItems.forEach(function(boxItem) {
	const linkBox = boxItem.querySelector('.link-box');
	const boxItemsBefore = boxItem.querySelector('.link-box:before');
	boxItem.addEventListener('mouseenter', function() {
		if (linkBox) {
			boxItemsBefore.style.border = '4px solid #fff';
			boxItemsBefore.style.zIndex = '11';
		}
	});
	boxItem.addEventListener('mouseleave', function() {
		if (linkBox) {
			boxItemsBefore.style.border = 'none';
			boxItemsBefore.style.zIndex = '1';
		}
	});
});
